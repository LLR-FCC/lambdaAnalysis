...
void NLLcombinedModel240() {

    // I m p o r t  W o r k s p a c e
    // ------------------------------

    TFile wsFile("/afs/cern.ch/user/n/niharrin/work/public/MA/Analysis/Fits/240/WScombinedModel_muZH.root");

    gROOT->cd();

    RooWorkspace *combWS = (RooWorkspace*)wsFile.Get("combWS_240");
 ...
 TCanvas* c5 = new TCanvas ("c5", "c5", 100, 200, 700, 500);
  ...
  RooAbsReal * nll = combModel_bare->createNLL(*asimov, Extended(kTRUE), Offset(kTRUE), NumCPU(8, 0), Range("range"), SplitRange(kTRUE)); //Best Result
