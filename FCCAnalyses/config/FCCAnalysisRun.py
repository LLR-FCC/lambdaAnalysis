...
ROOT.gSystem.Load("libFCCAnalysesHiggs") #inserted for custom libraries

ROOT.gErrorIgnoreLevel = ROOT.kFatal
#Is this still needed?? 01/04/2022 still to be the case
_fcc  = ROOT.dummyLoader
_higgs  = ROOT.dummyLoaderHiggs #inserted for custom libraries
...
def sendToBatch(rdfModule, chunkList, process, analysisFile):
    ...
    for ch in range(len(chunkList)):
        ...
        #frun.write('export PYTHONPATH={}:$PYTHONPATH\n'.format(localDir))
        #frun.write('export LD_LIBRARY_PATH={}/install/lib:$LD_LIBRARY_PATH\n'.format(localDir))
        #frun.write('export ROOT_INCLUDE_PATH={}/install/include/FCCAnalyses:$ROOT_INCLUDE_PATH\n'.format(localDir))

        frun.write('#!/bin/bash\n')
        frun.write('cd /afs/cern.ch/user/n/niharrin/work/public/MA/FCCAnalyses\n')
        frun.write('source setup2.sh\n')
        frun.write('source setup.sh\n')
        frun.write('cd /afs/cern.ch/user/n/niharrin/work/public/MA/FCCeePhysicsPerformance/case-studies/higgs/dataframe\n')
        frun.write('source localSetup.sh\n')
        frun.write('cd /afs/cern.ch/user/n/niharrin/work/public/MA/FCCAnalyses/BatchOutputs\n')
        ...
        if not os.path.isabs(outputDir):
            frun.write('python {}/config/FCCAnalysisRun.py {} --batch --output {}chunk{}.root --files-list '.format(localDir, analysisFile, outputDir, ch))
        else:
            frun.write('python {}/config/FCCAnalysisRun.py {} --batch --output {}{}/chunk{}.root --files-list '.format(localDir, analysisFile, outputDir, process,ch))
